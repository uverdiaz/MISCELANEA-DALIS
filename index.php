<!DOCTYPE html>
<html lang="es">

<head>
    <?php require_once("compartido/head.php") ?>
    <title>Papeleria DALIS J.E</title>
</head>

<body>
    <header>
        <?php require_once("compartido/nav.php") ?>

        <section class="textos-header">
            <h1>Recursos a la vuelta de tu esquina</h1>
            <h2>Variedades Dalis J.E</h2>
        </section>
        <div class="wave" style="height: 150px; overflow: hidden;"><svg viewBox="0 0 500 150" preserveAspectRatio="none" style="height: 100%; width: 100%;">
                <path d="M-19.74,64.63 C150.67,154.44 378.10,-52.78 520.87,52.78 L500.00,150.00 L0.00,150.00 Z" style="stroke: none; fill: #fff;"></path>
            </svg></div>
    </header>
    <main>
        <section class="conten-sobre-nosotros"></section>
        <h2 class="titulo">Nuestras novedades</h2>
        <div class="contenedor-sobre-nosotros">
            <img src="img/1.jpg" alt="" class="imagen-about-us">
            <div class="contenido-textos">
                <h3><span>1</span>Los mejores productos</h3>
                <p>
                    <font size="4">Cuando nos necesites podremos ofrecerte lo que desees, y te podremos ofrecer productos de gran calidad.</font>
                </p>

                <h3><span>2</span>Novedad</h3>
                <p>
                    <font size="4">Por cuarentena solo estamos abriendo de lunes a viernes de 9:00am a 2:00pm, para que vengas y podamos ofrecerte lo que necesites.</font>
                </p>


            </div>


        </div>
        <section class="portafolio">
            <div class="contenedor">
                <h2 class="titulo">Inventario</h2>
                <div class="galeria-port">
                    <div class="imagen-port">

                        <img src="img/in1.jpg" alt="">
                        <div class="hover-galeria">

                            <a href="papeleria.php"><img src="img/click.jpg" alt=""></a>
                            <p>Reglas</p>
                        </div>
                    </div>
                    <div class="imagen-port">
                        <img src="img/in2.jpg" alt="">
                        <div class="hover-galeria">
                            <a href="papeleria.php"><img src="img/click.jpg" alt=""></a>

                            <p>Libretas</p>
                        </div>
                    </div>
                    <div class="imagen-port">
                        <img src="img/inv3.jpg" alt="">
                        <div class="hover-galeria">
                            <a href="papeleria.php"><img src="img/click.jpg" alt=""></a>
                            <p>Carpetas</p>
                        </div>
                    </div>
                    <div class="imagen-port">
                        <img src="img/inv4.jpg" alt="">
                        <div class="hover-galeria">
                            <a href="papeleria.php"><img src="img/click.jpg" alt=""></a>
                            <p>Resmas</p>
                        </div>
                    </div>
                    <div class="imagen-port">
                        <img src="img/inv5.jpg" alt="">

                        <div class="hover-galeria">
                            <a href="papeleria.php"><img src="img/click.jpg" alt=""></a>
                            <p>Fomis</p>
                        </div>
                    </div>
                    <div class="imagen-port">
                        <img src="img/inv6.jpg" alt="">
                        <div class="hover-galeria">
                            <a href="papeleria.php"><img src="img/click.jpg" alt=""></a>
                            <p>Colores</p>
                        </div>
                    </div>
                    <div class="imagen-port">
                        <img src="img/inv7.png" alt="">
                        <div class="hover-galeria">
                            <a href="papeleria.php"><img src="img/click.jpg" alt=""></a>
                            <p>Marcadores</p>
                        </div>
                    </div>
                    <div class="imagen-port">
                        <img src="img/inve8.jpg" alt="">
                        <div class="hover-galeria">
                            <a href="papeleria.php"><img src="img/click.jpg" alt=""></a>
                            <p>Lapiceros</p>
                        </div>
                    </div>

                </div>


            </div>

        </section>
        <section class="about-services">
            <div class="contenedor">
                <h2 class="titulo">Lo que podemos ofrecerte</h2>
                <div class="servicio-cout">
                    <div class="servicio-ind">
                        <img src="img/ofre1.jpg" alt="">
                        <h3>Papeleria</h3>
                        <p>Una gran variedad de productos, que podras adquirir en nuestro establecimiento.</p>

                    </div>
                    <div class="servicio-ind">
                        <img src="img/ofre2.jpg" alt="">
                        <h3>Servicio de Copisteria</h3>
                        <p>Fotocopias e impresiones en B/N y COLOR - Impresiones digitales y demas.</p>

                    </div>
                    <div class="servicio-ind">
                        <img src="img/ofre3.jpg" alt="">
                        <h3>Llamadas y Recargas</h3>
                        <p>Llamadas locales y a movil, y recargas a todos los operadores deseados.</p>

                    </div>
                </div>
            </div>
        </section>





    </main>
    <?php require_once("compartido/footer.php") ?>



</body>

</html>